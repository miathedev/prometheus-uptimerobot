...
def fake_response():
    with mock.patch(
        "ws.prometheus_uptimerobot.web.UptimeRobotCollector" "._get_paginated"
    ) as page:
        page.return_value = json.loads(
            pkg_resources.resource_string(__name__, "response.json").decode("utf-8")
        )
        yield page
...
def test_converts_api_data_to_metrics(fake_response, testbrowser):
    ...
    ping_labels = (
        '{monitor_name="host ping",monitor_type="ping"' ',monitor_url="example.com"}'
    )
